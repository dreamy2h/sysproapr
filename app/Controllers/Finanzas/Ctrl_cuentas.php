<?php 
	...
	class Ctrl_cuentas extends BaseController {
		...
		public function guardar_cuenta() {
			...
			if ($email_cuenta == "") { $email_cuenta = null; }
			...
			if ($this->cuentas->save($datosCuenta)) {
				...
				if ($id_cuenta == "") {
					$obtener_id = $this->cuentas->select("max(id) as id_cuenta")->first();
		...
		public function llenar_cmb_banco() {
			$datosBancos = $this->bancos->select("id")->select("nombre_banco as banco")->findAll();
		...
		public function llenar_cmb_tipo_cuenta() {
			$datosTipoCuenta = $this->banco_tipo_cuenta->select("id")->select("glosa as tipo_cuenta")->findAll();
