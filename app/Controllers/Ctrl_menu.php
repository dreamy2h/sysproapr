<?php 
	...
	class Ctrl_menu extends BaseController {
		...
		public function permisos_usuario() {
			...
			$data = $this->permisos_usuario
			->select("pe.id as id_grupo")
			->select("pe.nombre as grupo")
			->select("pe.icono as icono_grupo")
			->select("pe.collapse")
		    ->select("pd.nombre as permiso")
		    ->select("pd.ruta")
		    ->select("pd.div_id")
		    ->select("pd.icono")
		    ->select("ps.id as id_subgrupo")
            ->select("ps.nombre as subgrupo")
            ->select("ps.icono as icono_subgrupo")
            ->select("ps.collapse as collapse_subgrupo")
			->join("permisos_detalle pd", "permisos_usuario.id_permiso = pd.id")
			->join("permisos_enc pe", "pd.id_grupo = pe.id")
            ->join("permisos_subgrupo ps", "pd.id_subgrupo = ps.id", "left")
			->where("permisos_usuario.estado", ACTIVO)
		    ->where("permisos_usuario.id_usuario", $this->sesión->id_usuario_ses)
			->orderBy("pe.orden")
			->orderBy("pd.orden")
			->orderBy("ps.id")
			->findAll();
