<?php 
	...
	class Ctrl_repactacion extends BaseController {
		...
		public function guardar_repactacion() {
			...
			$datosMetros = $this->metros->select("estado")->select("id as id_metros")->select("total_mes")->select("cuota_repactacion")->where("date_format(fecha_vencimiento, '%m-%Y')", $fecha_pago)->whereNotIn("estado", [0])->where("id_socio", $id_socio)->first();
			...
			$obtener_id = $this->repactaciones->select("max(id) as id_repactacion")->first();
