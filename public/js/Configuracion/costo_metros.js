...
function eliminar_costo_metros(observacion, id_costo_metros) {
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            if (respuesta == OK) {
                Toast.create("Éxito", "Costo eliminado con éxito", TOAST_STATUS.SUCCESS, 5000);
    ...
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            if (respuesta.respuesta == OK) {
                ...
                Toast.create("Éxito", "Costo de metros, guardado con éxito", TOAST_STATUS.SUCCESS, 5000);
            } else {
                Toast.create("Error", respuesta.respuesta, TOAST_STATUS.DANGER, 5000);
            }
        },
        error: function(error) {
            respuesta = JSON.parse(error["responseText"]);
            ...
            success: function(datos) {
                if (Object.keys(datos).length > 0) {
                    ...
                    if ($("#grid_costo_metros").DataTable().rows().count() > 0) {
                        Toast.create("Advertencia", "No se pudo cargar el cargo fijo", TOAST_STATUS.WARNING, 5000);
                    }
                }
            },
            error: function(error) {
...
$(document).ready(function() {
	...
	$("#btn_nuevo").on("click", function() {
        if ($("#cmb_apr").val() != "" && $("#cmb_diametro").val() != "") {
            ...
            Toast.create("Advertencia", "Seleccionar una APR y un diámetro", TOAST_STATUS.WARNING, 5000);
        }
    });
