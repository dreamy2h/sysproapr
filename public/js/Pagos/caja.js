...
function calcular_vuelto() {
    ...
    if (parseInt(total_pagar) > 0) {
        if (parseInt(entregado) < parseInt(total_pagar)) {
            Toast.create("Advertencia", "Lo entregado no puede ser menor al total a pagar", TOAST_STATUS.WARNING, 5000);
            $("#txt_entregado").val(0);
        } else {
        ...
        $("#txt_entregado").val(0);
        Toast.create("Advertencia", "Seleccione deudas a pagar", TOAST_STATUS.WARNING, 5000);
    }
}
        ...
        if (!isNaN(descuento)) {
            if (parseInt(total_pagar) > 0) {
                if (parseInt(descuento) > parseInt(total_pagar)) {
                    Toast.create("Advertencia", "El descuento no puede ser mayor al total a pagar", TOAST_STATUS.WARNING, 5000);
                    $("#txt_descuento").val(0);
                } else {
...
function guardar_pago() {
    ...
    if (parseInt(entregado) < parseInt(total_pagar)) {
        Toast.create("Error", "Lo entregado no puede ser menor al total a pagar", TOAST_STATUS.DANGER, 5000);
    } else {
        Swal.fire({
            title: "¿Efectuar Pago?",
                ...
                $.ajax({
                    ...
                    success: function(respuesta) {
                        ...
                        if (respuesta == OK) {
                            ...
                            Toast.create("Éxito", "Pago guardado con éxito", TOAST_STATUS.SUCCESS, 5000);
